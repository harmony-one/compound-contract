...
describe("RewardDistributor", () => {
    ...
    beforeEach(async () => {
        ...
        // impersonate users
        await ethers.provider.send("hardhat_impersonateAccount", [lenderAddress]);
        lender = await ethers.getSigner(lenderAddress);
        await ethers.provider.send("hardhat_impersonateAccount", [opWhaleAddress]);
        ...
        await opToken.connect(opWhale).transfer(rewardDistributor.address, ethers.utils.parseEther("100000"));
    });
    ...
    describe("Admin", () => {
        it("Should whitelist a token", async () => {
            await expect(rewardDistributor._whitelistToken(sonneAddress)).to.not.reverted;
        });

        it("Should revert non admin whitelist", async () => {
            const [deployer, user] = await ethers.getSigners();
            await expect(rewardDistributor.connect(user)._whitelistToken(sonneAddress)).to.revertedWith(
                "Ownable: caller is not the owner",
            );
        });

        it("Should revert whitelist zero address", async () => {
            await expect(rewardDistributor._whitelistToken(ethers.constants.AddressZero)).to.revertedWith(
        ...
        it("Should revert whitelist a token twice", async () => {
            ...
            await expect(rewardDistributor._whitelistToken(sonneAddress)).to.revertedWith(
        ...
        it("Should update reward speeds", async () => {
            ...
            await expect(
                rewardDistributor._updateRewardSpeeds(
                    opAddress,
                    ["0x5569b83de187375d43FBd747598bfe64fC8f6436", "0x5Ff29E4470799b982408130EFAaBdeeAE7f66a10"],
        ...
        it("Should revert update reward speeds incorrect input", async () => {
            ...
            ).to.revertedWith("RewardDistributor: borrow speed array length mismatch");
            ...
            ).to.revertedWith("RewardDistributor: supply speed array length mismatch");
        ...
        it("Should grant reward to user ", async () => {
            const [deployer, user] = await ethers.getSigners();
            await expect(rewardDistributor.connect(deployer)._grantReward(opAddress, deployer.address, 1000)).to.not
                .reverted;

            await expect(rewardDistributor.connect(user)._grantReward(opAddress, user.address, 1000)).to.revertedWith(
                "Ownable: caller is not the owner",
            );

            await expect(
                rewardDistributor.connect(deployer)._grantReward(lenderAddress, deployer.address, 1000),
            ).to.revertedWith("RewardDistributor: grant reward token does not exist");
        });

        it("Should revert non admin grant reward", async () => {
            const [deployer, user] = await ethers.getSigners();
            await expect(
                rewardDistributor.connect(user)._grantReward(opAddress, deployer.address, 1000),
            ).to.revertedWith("Ownable: caller is not the owner");
        });
    });
    ...
    it("Should claim external rewards", async () => {
        ...
        await comptroller.connect(user).functions["claimComp(address,address[])"](lenderAddress, userMarkets);
        //
        ...
        await ethers.provider.send("evm_setNextBlockTimestamp", [networkNow + halfPeriod]);
        ...
        // set block time to reward start
        await ethers.provider.send("evm_setNextBlockTimestamp", [networkNow + fullPeriod]);
        ...
        expect(diffOp).to.closeTo(expectedOpReward, ethers.utils.parseUnits("1", 9));
    });
});
...
const calculateRewards = async (start, end, comptroller, rewardDistributor) => {
    ...
    const marketStates = await Promise.all(
        positions.map((position) => rewardDistributor.rewardMarketState(opAddress, position.cToken.address)),
    );
    ...
    const totalReward = marketRewards.reduce((acc, reward) => acc.add(reward), ethers.BigNumber.from(0));

    return totalReward;
};

const getWalletPositions = async (comptroller: Contract, userAddress: string) => {
    const userMarkets = await comptroller.getAssetsIn(userAddress);
    const cTokens = await Promise.all(
        userMarkets.map((market: string) => ethers.getContractAt("CTokenInterface", market)),
    );
    const balances = await Promise.all(cTokens.map((cToken) => cToken.balanceOf(userAddress)));
    let borrowBalances = await Promise.all(cTokens.map((cToken) => cToken.borrowBalanceStored(userAddress)));
    const borrowIndexs = await Promise.all(cTokens.map((cToken) => cToken.borrowIndex()));
    const totalSupplies = await Promise.all(cTokens.map((cToken) => cToken.totalSupply()));
    let totalBorrows = await Promise.all(cTokens.map((cToken) => cToken.totalBorrows()));

    borrowBalances = borrowBalances.map((balance, i) => balance.mul(MANTISSA).div(borrowIndexs[i]));
    totalBorrows = totalBorrows.map((balance, i) => balance.mul(MANTISSA).div(borrowIndexs[i]));
...
const watchBalances = async (fn: Function, tokens: string[], userAddress: string) => {
    const tokenContracts = await Promise.all(tokens.map((token) => ethers.getContractAt("EIP20Interface", token)));
    const getBalances = async () =>
        await Promise.all(tokenContracts.map((contract) => contract.balanceOf(userAddress)));
